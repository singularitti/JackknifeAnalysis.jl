...
We also plot the time average of the virial in Figure~\ref{fig:MD:b},
for each time step \(m\), it is represented by
%
\begin{equation}
    \langle \bm{r} \cdot \bm{F} \rangle (M) =
    \frac{ 1 }{ M } \sum_{m=1}^{M}
    \sum_{i=1}^N \langle \bm{r}_i(m) \cdot \bm{F}_i(m) \rangle,
\end{equation}
%
where \(i\) labels each particle, and \(m\) labels each time step.
...
\begin{figure}[H]
    ...
    \begin{minipage}[t]{0.8\linewidth}
        ...
        \subcaption{The virial and time-averaged virial for the
            Argon molecular dynamics from simulation time from \(t = 480\) to \(770\).}
        \label{fig:MD:b}
    \end{minipage}
