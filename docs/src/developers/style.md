...
!!! info
    The style guide is always a work in progress, and not all JackknifeAnalysis code
    follows the rules. When modifying JackknifeAnalysis, please fix the style violations
...
`JackknifeAnalysis` uses [`JuliaFormatter`](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/JackknifeAnalysis.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the `JackknifeAnalysis` directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to `JackknifeAnalysis` have
