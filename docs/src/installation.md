...
[`JackknifeAnalysis`](https://github.com/singularitti/JackknifeAnalysis.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("JackknifeAnalysis")
   ```

3. Run

   ```julia-repl
   julia> using JackknifeAnalysis
   ```
...
version of `JackknifeAnalysis`, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/JackknifeAnalysis.jl"
```

in the second step above.

## Update `JackknifeAnalysis`
...
our [GitHub repository](https://github.com/singularitti/JackknifeAnalysis.jl)
for new releases.
Once we release a new version, you can update `JackknifeAnalysis` by typing

```@repl
using Pkg
Pkg.update("JackknifeAnalysis")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall `JackknifeAnalysis`
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("JackknifeAnalysis")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall `JackknifeAnalysis`.
